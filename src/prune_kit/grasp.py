"""Gradient Signal Preservation (GraSP) pruning at initialization.

GraSP (Wang, Zhang, Grosse; ICML 2020) scores every scalar connection by
how much removing it would change the loss gradient:

    s_j = - w_j * (H g)_j

where ``g = ∇L`` and ``H`` is the Hessian (or a Hessian-vector product
``Hg`` evaluated on one minibatch at initialization). Connections with
the **largest** scores are kept — they preserve the most gradient signal.
``density`` is the fraction to keep; rounding matches the magnitude and
SNIP pruners (``round(density * N)``). Ties break toward the earlier
layer in ``specs`` order, then the lower index.

Pass either ``hg`` (the Hessian-vector product with the gradient) or
precomputed ``contributions`` (already ``w * (Hg)`` or ``(Hg)`` terms
whose absolute product form is handled below). When ``contributions``
are given they are treated as the raw ``w * (Hg)`` product and negated
to form the GraSP score. Sequences may be lists or any array-like
buffer; values are read with ``float`` so a NumPy array works without
taking a NumPy dependency.
"""
...
def _grasp_terms(
    ...
    hg: Sequence[float] | None,
    contributions: Sequence[float] | None,
) -> List[float]:
    """GraSP scores ``-w * (Hg)`` (or ``-contribution`` when given)."""
    if contributions is not None and hg is not None:
        raise ValueError("pass hg or contributions, not both")
    if contributions is None and hg is None:
        raise ValueError("hg or contributions is required")
    if contributions is not None:
        ...
        # Caller-supplied w*(Hg) (or Hg already multiplied); negate for GraSP.
        return [-float(value) for value in contributions]
    assert hg is not None
    if len(hg) != len(weights):
        raise ValueError(
            f"hg has {len(hg)} values, weights has {len(weights)}"
        )
    return [
        -float(weight) * float(h)
        for weight, h in zip(weights, hg)
    ]


def grasp_scores(
    weights: Sequence[float],
    hg: Sequence[float] | None = None,
    *,
    contributions: Sequence[float] | None = None,
    shape: Sequence[int] | None = None,
) -> List[float]:
    """GraSP score of each weight, same length as ``weights``.

    Each score is ``-weight * hg``. ``contributions``, when given,
    replaces that product with a caller-supplied per-connection term
    (already ``w * (Hg)``); the value is negated. Higher scores are kept.
    ...
    return _grasp_terms(weights, hg, contributions)
...
def grasp_prune_layer(
    ...
    *,
    ...
    """Zero the lowest-GraSP-score connections in one layer.
...
def _resolve_saliency(
    hg: Dict[str, Sequence[float]] | None,
...
def _layer_scores(
    ...
    hg = values if source == "hg" else None
...
def _validate_model_args(
    ...
    source, saliency = _resolve_saliency(hg, contributions)
...
def _grasp_prune_detailed(
    ...
    density, scope, source, saliency = _validate_model_args(
        specs, model, hg, contributions, density, scope, per_layer
    )
    scores = {
...
def grasp_prune_model(
    specs: Sequence[LayerSpec],
    model: Dict[str, Sequence[float]],
    hg: Dict[str, Sequence[float]] | None = None,
    *,
    ...
    """One-shot GraSP prune of every dense and conv layer.

    ``scope="global"`` keeps the top ``density`` fraction of connections
    across the whole model. ``scope="layer"`` ranks inside each layer;
    ``per_layer`` then overrides the density for named layers.
    """
    pruned, _, _, _, _ = _grasp_prune_detailed(
        specs, model, hg, contributions, density, scope, per_layer
    )
    return pruned


def grasp_prune_summary(
    specs: Sequence[LayerSpec],
    model: Dict[str, Sequence[float]],
    hg: Dict[str, Sequence[float]] | None = None,
    *,
    ...
    """Run GraSP and return per-layer keep counts, masks, and score totals.
    ...
    the GraSP scores ``-w * (Hg)``.
    """
    pruned, masks, scores, density, scope = _grasp_prune_detailed(
        specs, model, hg, contributions, density, scope, per_layer
    )
